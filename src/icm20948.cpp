...
void icm20948::init()
{
  ...
  reset_icm20948();
  // reset_mag();
  // if (whoami() != who_am_i_content) {
  // return hal::new_error(std::errc::no_such_device); # TODO: Implement timeout
...
void icm20948::set_gyro_offsets(const gyro_offset_t& gyr_offsets)
{
...
icm20948::mag_read_t icm20948::read_magnetometer()
{
  ...
  while (true) {
    ...
    // if (++polling_attempts > max_polling_attempts) { # TODO: Implement
...
icm20948::temp_read_t icm20948::read_temperature()
{
  ...
  switch_bank(0);
  data = hal::write_then_read<2>(*m_i2c,
                                 icm20948_address,
...
void icm20948::switch_bank(hal::byte p_newBank)
{
...
hal::byte icm20948::read_register16(hal::byte p_bank, hal::byte p_reg)
{

  switch_bank(p_bank);
  hal::byte reg16Val = 0;

  auto MSByte = hal::write_then_read<1>(*m_i2c,
                                        icm20948_address,
                                        std::array<hal::byte, 1>{ p_reg },
                                        hal::never_timeout());
  auto LSByte = hal::write_then_read<1>(*m_i2c,
                                        icm20948_address,
                                        std::array<hal::byte, 1>{ p_reg },
                                        hal::never_timeout());

  reg16Val = (MSByte[0] << 8) + LSByte[0];
  return reg16Val;
}
